...
public class TC_Doctor_Handover_RecordPerPage {
  ...
  public void f() {
	  ...
	  String rcount = pageObjects.DoctorHandoverSignOutSheetPage.RecordCount(driver).getText();
	  //PageNavigation_Action.Execute(driver, pageObjects.DoctorHandoverSignOutSheetPage.Recordpp(driver), rcount);
	 /* String rcount = pageObjects.WardRoundSheetPage.RecordCount(driver).getText();
	  ...
	  for (int i=0;i<=5;i++) {//for records per page dropdown
		  ...
		  for (int j=1;j<(pagecount+1);j++) { //for number of pages calculation
		   ...
		   String ddselection="utility.Constant.DDSelect_"+[i];
		  int divisor = Integer.parseInt(ddselection);
		  int pagecount = (int) Math.ceil(count/divisor);
		  System.out.println(pagecount);
	  ...
	  String[] ddcount = pageObjects.DoctorHandoverSignOutSheetPage.Recordpp(driver).getText().split("\n");
	  ...
	  for(int i=0; i<ddc; i++) {
		  ...
		  PageNavigation_Action.Execute1(driver, pagecount, dd_value, pageObjects.DoctorHandoverSignOutSheetPage.Recordpp(driver));
